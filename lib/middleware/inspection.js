...
function inspection( req, res, next ) {
	...
	if( `${ req.protocol }://${ req.hostname }${ req.path }`.length >= maximumURISize ) {
		return res.locals.error( new Response( 414, 'URI exceeds maximum length' ) );
	} else if( req.rawHeaders.join( '' ).length >= maximumHeaderSize ) {
		return res.locals.error( new Response( 431, 'Request Header Fields Too Large' ) );
	} else if( req.headers[ 'content-length' ] >= maximumPayloadSize ) {
		return res.locals.error( new Response( 413, 'Payload Too Large' ) );
	} else if( +req.httpVersion < minimumHTTPVersion ) {
		return res.locals.error( new Response( 505, 'HTTP Version Not Supported' ) );
	}
