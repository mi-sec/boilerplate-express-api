...
function authorization( req, res, next ) {
    ...
    if( !p.config ) {
        // return p.error( new Response( 405, `Method: ${req.path} not allowed` ) );
    ...
    jwtoken
        .then(
            token => {
                if( p.hasAuthorization ) {
                    ...
                    } else if( p.config.permission && !token.permissions.includes( p.config.permissions ) ) {
                        return p.error( APIError.NOT_AUTHORIZED );
                    } else if( p.config.permission ) {
        ...
        .catch( e => {
            if( e instanceof Response ) {
                ...
                return p.error( new Response( 500, e ) );
            }
        } );
}
